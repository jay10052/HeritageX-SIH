...
// GLOBAL STATE AND MODAL INTERACTION LOGIC
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (exploreMain) {
        exploreMain.addEventListener('click', (event) => {
            let card = event.target.closest('.info-card');
    ...
    function openDetailsModal(key) {
        ...
        modalContent.innerHTML = `
            <img id="modalImage" src="${data.image}" alt="${data.title}" class="modal-img">
            <div class="modal-text-content">
                ...
                <a href="#" class="modal-action-btn btn btn-danger btn-lg">View Virtual Tour</a>
            </div>
        `;
        const virtualTourButton = modalContent.querySelector('.modal-action-btn');
        if (virtualTourButton) {
            virtualTourButton.onclick = openVirtualTour; 
        }
    ...
    // Function to open the Virtual Tour Video Modal
    window.openVirtualTour = function() {
        const data = exploreData[currentSelectedKey];
        if (!data || !data.videoUrl) {
            alert("Virtual Tour content is not yet available for this item!");
            return;
        }

        const videoModal = document.getElementById("videoModal");
        const videoFrame = document.getElementById("virtualTourFrame");
        const videoTitle = document.getElementById("modalVideoTitle");
        document.getElementById('detailsModal').style.display = 'none';

    
        videoTitle.textContent = "Virtual Tour: " + data.title.split(":")[0]; 
        videoFrame.src = data.videoUrl; 
        videoModal.style.display = "flex"; 
    }

    // Function to close the Virtual Tour Video Modal and STOP the video
    ...
    //Global click event to close modals
    ...
    // --- 5. Hamburger Menu & Active Link Logic
    ...
    navItems.forEach(link => {
        ...
        if (linkHref === 'index.html' && (fileName === 'index.html' || fileName === '')) {
            link.classList.add('active');
        } 
        else if (linkHref === fileName && fileName !== '') {
            link.classList.add('active');
        }
    });
